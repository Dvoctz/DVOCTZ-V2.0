...
export default function HomePage() {
  ...
  const [sponsors, setSponsors] = useState<{ id: string, name: string, logo_url: string }[]>([])
  const [div1Champion, setDiv1Champion] = useState<{ teamName: string, tournamentName: string, tournamentId: number } | null>(null)
  const [div2Champion, setDiv2Champion] = useState<{ teamName: string, tournamentName: string, tournamentId: number } | null>(null)

  useEffect(() => {
    const loadHomeData = async () => {
      const [tRes, uRes, rRes, sRes] = await Promise.all([
        supabase.from('tournaments').select('*').order('id', { ascending: false }),
        supabase.from('fixtures').select('id, tournament_id, team1_id, team2_id, status, date_time, ground, best_of, team1:teams!team1_id(name, logo_url), team2:teams!team2_id(name, logo_url), tournaments(name, division, phase)').eq('status', 'upcoming').order('date_time', { ascending: true }).limit(20),
        supabase.from('fixtures').select('id, tournament_id, team1_id, team2_id, status, date_time, ground, best_of, score, winner:teams!winner_team_id(name), team1:teams!team1_id(name, logo_url), team2:teams!team2_id(name, logo_url), tournaments(name, division, phase)').eq('status', 'completed').order('date_time', { ascending: false }).limit(5),
      ...
      const latestDiv1 = loadedTournaments.find(t => t.division === 'Division 1' && t.phase === 'completed')
      const latestDiv2 = loadedTournaments.find(t => t.division === 'Division 2' && t.phase === 'completed')

      const fetchChampion = async (tournament: Tournament) => {
        const { data: fixtures } = await supabase
          .from('fixtures')
          .select('*, team1:teams!team1_id(name), team2:teams!team2_id(name), winner:teams!winner_team_id(name)')
        ...
        if (tournament.phase === 'knockout') {
          ...
          if (finalMatch && finalMatch.winner) {
            return { teamName: finalMatch.winner.name, tournamentName: tournament.name, tournamentId: tournament.id }
          }
          // Fallback if no specific 'final' stage found, try last match
          const lastMatch = fixtures.sort((a, b) => new Date(b.date_time).getTime() - new Date(a.date_time).getTime())[0]
          if (lastMatch && lastMatch.winner) {
             return { teamName: lastMatch.winner.name, tournamentName: tournament.name, tournamentId: tournament.id }
          }
        } else {
          // Round Robin Standings calculation inline
          const table = new Map<number, { name: string, points: number, diff: number }>()
          fixtures.forEach(f => {
            if (!table.has(f.team1_id) && f.team1_id) table.set(f.team1_id, { name: f.team1?.name || 'Unknown', points: 0, diff: 0 })
            if (!table.has(f.team2_id) && f.team2_id) table.set(f.team2_id, { name: f.team2?.name || 'Unknown', points: 0, diff: 0 })
          ...
          if (standings.length > 0) {
            return { teamName: standings[0].name, tournamentName: tournament.name, tournamentId: tournament.id }
          }
        }
        return null
      }
  ...
  const renderUpcomingCard = (f: FixturePreview) => (
    <div key={f.id} className="group p-5 border border-zinc-900 bg-zinc-950 hover:bg-zinc-900/80 transition-colors flex flex-col justify-between">
       <div className="flex justify-between items-center mb-5">
         <span className={`text-[10px] uppercase font-bold tracking-widest px-2 py-0.5 rounded-sm border ${f.tournaments?.division === 'Division 1' ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}>
         ...
         <div className="flex-1 min-w-0 text-center flex flex-col items-center justify-center">
            {f.team1?.logo_url ? (
              <div className="w-8 h-8 mb-2 bg-white/5 rounded-full p-1 border border-zinc-800 flex items-center justify-center overflow-hidden">
                <img src={f.team1.logo_url} alt={f.team1.name} className="w-full h-full object-contain" />
              </div>
            ) : (
              <div className="w-8 h-8 mb-2 bg-zinc-900 border border-zinc-800 rounded-full flex items-center justify-center">
                <span className="text-[10px] font-bold text-zinc-500 uppercase">{f.team1?.name?.substring(0, 2) || 'T1'}</span>
              </div>
            )}
            <span className="block text-sm font-bold text-white truncate w-full">{f.team1?.name || 'TBD'}</span>
         </div>
         <span className="text-[10px] text-zinc-600 font-bold uppercase shrink-0">vs</span>
         <div className="flex-1 min-w-0 text-center flex flex-col items-center justify-center">
            {f.team2?.logo_url ? (
              <div className="w-8 h-8 mb-2 bg-white/5 rounded-full p-1 border border-zinc-800 flex items-center justify-center overflow-hidden">
                <img src={f.team2.logo_url} alt={f.team2.name} className="w-full h-full object-contain" />
              </div>
            ) : (
              <div className="w-8 h-8 mb-2 bg-zinc-900 border border-zinc-800 rounded-full flex items-center justify-center">
                <span className="text-[10px] font-bold text-zinc-500 uppercase">{f.team2?.name?.substring(0, 2) || 'T2'}</span>
              </div>
            )}
            <span className="block text-sm font-bold text-white truncate w-full">{f.team2?.name || 'TBD'}</span>
         </div>
       </div>
  ...
  return (
    <div className="flex flex-col flex-1">
      ...
      <section className="relative px-6 md:px-10 py-24 flex flex-col gap-10 overflow-hidden flex-1 border-b border-zinc-900 bg-black">
        ...
        <header className="relative z-10 max-w-3xl">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Link to="/#circuits" className="w-full sm:w-auto">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto relative z-10">
             {div1Champion && (
               <div className="bg-zinc-950/80 border border-amber-500/30 p-8 text-center relative group overflow-hidden hover:border-amber-500 transition-colors">
                 ...
                 <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500 font-bold mb-2">Division 1 Current Champions</p>
                 <h3 className="text-3xl md:text-4xl font-black italic text-white mb-4 drop-shadow-sm">{div1Champion.teamName}</h3>
                 <p className="text-xs text-zinc-400 mb-6 font-medium">{div1Champion.tournamentName}</p>
                 <Link to={`/tournaments/${div1Champion.tournamentId}`}>
                   <Button variant="outline" size="sm" className="border-amber-500/50 text-amber-500 hover:bg-amber-500 hover:text-black uppercase tracking-widest text-[10px] font-bold">
             ...
             {div2Champion && (
               <div className="bg-zinc-950/80 border border-amber-500/30 p-8 text-center relative group overflow-hidden hover:border-amber-500 transition-colors">
                 <div className="absolute inset-0 bg-amber-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                 <p className="text-[10px] uppercase tracking-[0.3em] text-amber-500 font-bold mb-2">Division 2 Current Champions</p>
                 <h3 className="text-3xl md:text-4xl font-black italic text-white mb-4 drop-shadow-sm">{div2Champion.teamName}</h3>
                 <p className="text-xs text-zinc-400 mb-6 font-medium">{div2Champion.tournamentName}</p>
      ...
      {(activeDiv1 || activeDiv2) && (
        <section id="standings" className="bg-black border-b border-zinc-900 p-6 md:p-10 flex flex-col flex-1">
            ...
            {tournaments.map((t) => (
              <Card key={t.id} className="group overflow-hidden hover:border-amber-500/50 border-zinc-900 transition-colors p-0 flex flex-col bg-black">
                <div className="aspect-video bg-zinc-900 border-b border-zinc-800 flex items-center justify-center relative overflow-hidden">
                  {t.banner_url ? (
                    <img src={t.banner_url} alt={t.name} className="absolute inset-0 w-full h-full object-cover z-0" />
                  ) : (
                    <>
      ...
      {sponsors.length > 0 && (
        <section className="bg-black py-16 border-t border-zinc-900 border-b">
          <div className="max-w-[1200px] mx-auto px-6 md:px-10">
            <h3 className="text-xs font-black tracking-widest uppercase text-zinc-600 text-center mb-10">Official Partners</h3>
            <div className="flex flex-wrap justify-center items-center gap-8 md:gap-16 opacity-50 hover:opacity-100 transition-opacity duration-500">
              {sponsors.map(sponsor => (
                <div key={sponsor.id} className="w-24 md:w-32 h-16 flex items-center justify-center grayscale hover:grayscale-0 transition-all duration-300">
                  {sponsor.logo_url ? (
                     <img src={sponsor.logo_url} alt={sponsor.name} className="max-w-full max-h-full object-contain mix-blend-screen" />
                  ) : (
                     <span className="text-sm font-bold text-zinc-500 uppercase tracking-widest">{sponsor.name}</span>
                  )}
                </div>
              ))}
            </div>
