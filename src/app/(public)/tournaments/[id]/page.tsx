...
export default function TournamentDetailPage() {
    ...
    const loadTournamentData = async () => {
      ...
      try {
        const [tRes, fRes] = await Promise.all([
          ...
          supabase.from("fixtures").select("id, team1_id, team2_id, status, stage, date_time, ground, best_of, referee, score, team1:teams!team1_id(name, logo_url), team2:teams!team2_id(name, logo_url), winner:teams!winner_team_id(name)").eq("tournament_id", id).order("date_time", { ascending: true })
        ])
  ...
  const renderLiveCard = (f: Fixture) => (
    <div key={f.id} className="p-5 border-2 border-amber-500/50 bg-black hover:bg-zinc-950 transition-colors relative overflow-hidden shadow-[0_0_15px_rgba(245,158,11,0.1)]">
      ...
      <div className="flex justify-between items-start mb-6 relative z-10">
        <div className="flex items-center gap-2">
           ...
           <span className="text-[10px] uppercase font-black tracking-widest text-amber-500">Live Now</span>
        ...
        <div className="text-right">
          <span className="block text-[10px] font-mono text-zinc-400">
            {new Date(f.date_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          </span>
          <span className="block text-[10px] text-zinc-500 mt-0.5">{f.ground}</span>
        </div>
      </div>
      ...
      {f.score?.sets && f.score.sets.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2 relative z-10">
          {f.score.sets.map((set, idx) => (
            <div key={idx} className="flex flex-col items-center bg-zinc-900/80 px-4 py-2 rounded-sm border border-zinc-800">
               <span className="text-[9px] text-zinc-500 uppercase font-black tracking-widest mb-1.5">Set {idx + 1}</span>
               <span className="text-sm font-bold text-white leading-none">{set.team1Points}</span>
               <div className="w-full h-px bg-zinc-800 my-1.5" />
               <span className="text-sm font-bold text-white leading-none">{set.team2Points}</span>
            </div>
          ))}
        </div>
      )}
  ...
  const renderBracketCard = (f: Fixture) => {
    ...
    return (
      <div key={f.id} className={`w-full p-4 border relative overflow-hidden transition-all ${t1Winner || t2Winner ? 'border-amber-500/30 bg-zinc-950/80 shadow-[0_0_15px_rgba(245,158,11,0.05)]' : isLive ? 'border-amber-500 bg-amber-500/5 shadow-[0_0_15px_rgba(245,158,11,0.15)]' : 'border-zinc-800 bg-zinc-950 hover:border-zinc-700'}`}>
        <div className="flex justify-between items-center mb-4">
           <span className={`text-[8px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm border ${isLive ? 'bg-amber-500/20 text-amber-500 border-amber-500/30 animate-pulse' : isUpcoming ? 'bg-zinc-900 text-zinc-500 border-zinc-800' : 'bg-zinc-800/80 text-zinc-400 border-zinc-700'}`}>
             {isLive ? 'LIVE' : isUpcoming ? 'Upcoming' : 'Result'} • BO{f.best_of}
           </span>
           <span className="text-[9px] font-mono text-zinc-500">
             {new Date(f.date_time).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
           </span>
        </div>
        <div className="flex flex-col gap-2">
          <div className={`flex justify-between items-center p-2 rounded-sm border transition-colors ${t1Winner ? 'bg-amber-500/10 border-amber-500/20' : 'bg-zinc-900/50 border-zinc-800/50 hover:bg-zinc-900'}`}>
            <div className="flex items-center gap-2 overflow-hidden">
              {f.team1?.logo_url ? (
                <img src={f.team1.logo_url} className="w-4 h-4 object-contain" alt="" />
              ) : (
                <div className="w-4 h-4 bg-zinc-800 rounded-full" />
              )}
              <span className={`text-xs font-bold truncate ${t1Winner ? 'text-amber-500' : 'text-zinc-300'}`}>{f.team1?.name || 'TBD'}</span>
            </div>
            <span className={`text-sm font-black tabular-nums pl-2 ${t1Winner ? 'text-amber-500' : 'text-zinc-500'}`}>{f.score?.team1Score ?? '-'}</span>
          </div>
          <div className={`flex justify-between items-center p-2 rounded-sm border transition-colors ${t2Winner ? 'bg-amber-500/10 border-amber-500/20' : 'bg-zinc-900/50 border-zinc-800/50 hover:bg-zinc-900'}`}>
            <div className="flex items-center gap-2 overflow-hidden">
              {f.team2?.logo_url ? (
                <img src={f.team2.logo_url} className="w-4 h-4 object-contain" alt="" />
              ) : (
                <div className="w-4 h-4 bg-zinc-800 rounded-full" />
              )}
              <span className={`text-xs font-bold truncate ${t2Winner ? 'text-amber-500' : 'text-zinc-300'}`}>{f.team2?.name || 'TBD'}</span>
            </div>
            <span className={`text-sm font-black tabular-nums pl-2 ${t2Winner ? 'text-amber-500' : 'text-zinc-500'}`}>{f.score?.team2Score ?? '-'}</span>
          </div>
        </div>
      </div>
    )
  ...
  const renderBracketColumn = (title: string, stageFilters: string[]) => {
    const columnFixtures = fixtures.filter(f => stageFilters.includes(f.stage || 'round-robin'));
    ...
    return (
      <div className="flex flex-col flex-1 min-w-[280px] snap-center">
        <div className="mb-6 pb-3 border-b-2 border-amber-500/20 text-center">
           <h3 className="text-xs font-black uppercase tracking-widest text-amber-500">{title}</h3>
        </div>
        <div className="flex flex-col gap-8 justify-around flex-1 relative h-full">
           {columnFixtures.length === 0 ? (
             <div className="p-6 border border-dashed border-zinc-800 text-center bg-zinc-950/50 min-h-[140px] flex items-center justify-center">
               <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-bold">TBD</span>
             </div>
           ) : (
  ...
  return (
    <div className="flex flex-col flex-1 pb-20">
      <div className="px-6 md:px-10 pt-10 pb-6">
        ...
        <section className={`mb-16 relative overflow-hidden border border-zinc-900 ${tournament.banner_url ? '' : 'p-8'}`}>
          {tournament.banner_url ? (
            <div className="relative h-64 md:h-80 w-full flex items-end p-8">
              <img src={tournament.banner_url} alt={tournament.name} className="absolute inset-0 w-full h-full object-cover z-0" />
            ...
            {fixtures.length === 0 ? (
              ...
              <div className="space-y-12">
                {FIXTURE_STAGES.map(stage => {
                  const stageFixtures = fixtures.filter(f => (f.stage || 'round-robin') === stage);
                  if (stageFixtures.length === 0) return null;
                  
                  const liveFixtures = stageFixtures.filter(f => f.status === 'live');
                  const upcomingFixtures = stageFixtures.filter(f => f.status === 'upcoming');
                  const completedFixtures = stageFixtures.filter(f => f.status === 'completed');

                  const isKnockoutStage = stage !== 'round-robin';
                  
                  return (
                    <div key={stage} className={`space-y-6 relative ${isKnockoutStage && tournament.phase === 'knockout' ? 'bg-zinc-900/10 border border-zinc-800/50 p-4 -mx-4' : ''}`}>
                      <h3 className={`text-xs font-black uppercase tracking-widest border-b pb-2 flex items-center gap-2 ${isKnockoutStage && tournament.phase === 'knockout' ? 'text-amber-500 border-amber-500/20' : 'text-zinc-300 border-zinc-800'}`}>
                      ...
                      <div className="space-y-8">
                        ...
                        {upcomingFixtures.length > 0 && (
                          <div>
                            <h4 className="text-[10px] uppercase tracking-widest text-zinc-400 font-bold mb-3">Upcoming Matches</h4>
                            <div className="space-y-4">
                        ...
                        {completedFixtures.length > 0 && (
                          <div>
                            <h4 className="text-[10px] uppercase tracking-widest text-zinc-400 font-bold mb-3">Completed Matches</h4>
                            <div className="space-y-4">
