...
export function TournamentTeamsManager({
  ...
  const handleAssign = async (e: any) => {
    ...
    const exists = tournamentTeams.some(
      (tt) =>
        ...
        tt.team_id.toString() === selectedTeam,
    );
    if (exists) {
  ...
  const selectedTournamentDivision = selectedTournamentObj?.division;
  ...
  const availableTeams = teams.filter(
    (t) =>
      ...
      (!selectedTournamentDivision ||
        t.division === selectedTournamentDivision),
  );
            ...
            <>
              ...
              >
                ...
                <Button
                  type="submit"
                  disabled={
                    isSubmitting || !selectedTeam || availableTeams.length === 0
                  }
                  className="bg-amber-500 text-black hover:bg-amber-400 h-[46px] px-8"
                >
                ...
                {assignedTeams.length === 0 ? (
                  <div className="text-center py-8 border border-dashed border-zinc-800">
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                      No teams enrolled in this tournament
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {assignedTeams.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center justify-between p-3 border border-zinc-800 bg-zinc-900/50 group"
                      >
                        <div>
