...
const FIXTURE_STATUSES = ["upcoming", "live", "completed"];
...
export function FixturesManager({
  ...
  const [tournaments, setTournaments] = useState<
    { id: string | number; name: string; division?: string; phase?: string }[]
  >([]);
  ...
  const displayedFixtures = tournamentFilter
    ? fixtures.filter((f) => {
  ...
  const loadData = async () => {
    ...
    try {
      const [fixturesRes, tournamentsRes, teamsRes, playersRes] =
        await Promise.all([
          supabase
            .from("fixtures")
            .select(
  ...
  const handleOpenModal = (f?: Fixture) => {
    if (f) {
      ...
      const validTournaments = displayedTournaments.filter(
        (t) => t.phase !== "completed",
      );
      ...
      if (
        !defaultTournamentId ||
        !validTournaments.some((t) => t.id.toString() === defaultTournamentId)
      ) {
        defaultTournamentId =
          validTournaments.length > 0 ? validTournaments[0].id.toString() : "";
      }
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (formData.winner_team_id) {
      if (
        formData.winner_team_id !== formData.team1_id &&
        formData.winner_team_id !== formData.team2_id
      ) {
    ...
    const t1Name =
      teams.find((t) => t.id.toString() === formData.team1_id)?.name ||
      "Team 1";
    const t2Name =
      teams.find((t) => t.id.toString() === formData.team2_id)?.name ||
      "Team 2";
  ...
  const selectedTournamentForFixture = tournaments.find(
    (t) => t.id.toString() === formData.tournament_id,
  );
  const selectedTournamentDivision = selectedTournamentForFixture?.division;
  const availableTeamsForFixture = teams.filter(
    (t) =>
      !selectedTournamentDivision || t.division === selectedTournamentDivision,
  );
    ...
    <section className="border border-zinc-800 bg-zinc-950 p-4 md:p-8 flex flex-col relative w-full overflow-hidden mt-6 md:mt-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-6 md:mb-8">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <Button
            ...
            className="w-full md:w-auto text-xs border-purple-500/20 bg-purple-500/10 text-purple-500 hover:bg-purple-500 hover:text-black disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 border border-dashed border-zinc-800 bg-zinc-900/20">
          ...
          <p className="text-[10px] text-zinc-500 max-w-md mx-auto mb-4">
            You must select a tournament from the dropdown above to view and
            manage its fixtures.
          </p>
        </div>
        ...
        <div className="text-center py-12 border border-dashed border-zinc-800">
          ...
          {searchQuery || tournamentFilter || statusFilter ? (
            <Button
              ...
              variant="outline"
              size="sm"
              className="border-purple-500/20 text-purple-500 hover:bg-purple-500 hover:text-black hover:border-purple-500"
            >
              Clear Filters
            </Button>
          ) : (
        ...
        <div className="space-y-8">
          {FIXTURE_STAGES.map((stage) => {
            const stageFixtures = displayedFixtures.filter(
                      ...
                      <div className="flex items-start md:items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <h3 className="text-base sm:text-lg font-medium text-white flex items-center gap-2 flex-wrap">
                            ...
                            <span
                              className={
                                !f.team2?.name ? "text-zinc-500 italic" : ""
                              }
                            >
                              {f.team2?.name || "TBD"}
                            </span>
                        ...
                        {f.tournaments?.phase === "completed" ? (
                          ...
                          <>
                            <Button
                              ...
                              className="h-8 px-2 text-zinc-400 hover:text-purple-500"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-zinc-950 border border-purple-500/30 shadow-[0_0_50px_rgba(0,0,0,0.8)] p-8 ring-4 ring-zinc-950/50 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6">
              <div>
                <p className="text-[10px] uppercase tracking-widest text-purple-500 font-bold mb-1">
                  Database Op
                </p>
                <h3 className="text-2xl font-bold tracking-tight italic text-white">
                  {editingId ? "Modify Fixture" : "Schedule Fixture"}
                </h3>
              </div>
              <button
                onClick={handleCloseModal}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  >
                    ...
                    {displayedTournaments
                      .filter(
                        (t) =>
                          t.phase !== "completed" ||
                          t.id.toString() === formData.tournament_id,
                      )
                      .map((t) => (
                        <option key={t.id} value={t.id}>
                          {t.name}
                  ...
                  >
                    {FIXTURE_STATUSES.map((status) => (
                      <option
                        key={status}
                        value={status}
                        className="bg-zinc-900 text-white uppercase"
                      >
                        {status}
                      </option>
                    ))}
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, stage: e.target.value }))
                    }
                  >
                    {FIXTURE_STAGES.map((stage) => (
                      <option
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border border-zinc-900 p-4 bg-zinc-900/20">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, team1_id: e.target.value }))
                    }
                  >
                    <option value="">TBD</option>
                    {availableTeamsForFixture.map((t) => (
                      <option key={t.id} value={t.id}>
                        {t.name}
                      </option>
                    ))}
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, team2_id: e.target.value }))
                    }
                  >
                    <option value="">TBD</option>
                    {availableTeamsForFixture.map((t) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, date_time: e.target.value }))
                    }
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, ground: e.target.value }))
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({ ...p, referee: e.target.value }))
                    }
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData((p) => ({
                        ...p,
                        man_of_the_match_id: e.target.value,
                      }))
                    }
                  >
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setFormData((p) => ({
                          ...p,
                          best_of: val,
                          score_sets: p.score_sets.slice(0, val),
                        }));
                      }}
                    >
                ...
                <div className="space-y-3">
                  {formData.score_sets.map((set, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <div className="w-8 text-[10px] font-bold text-zinc-500 uppercase">
                        Set {idx + 1}
                      </div>
                      <Input
                        type="number"
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const newSets = formData.score_sets.filter(
                            (_, i) => i !== idx,
                          );
                          setFormData({ ...formData, score_sets: newSets });
                        }}
                  ...
                  >
                    ...
                    {formData.team1_id && (
                      <option value={formData.team1_id}>
                        {teams.find(
                          (t) => t.id.toString() === formData.team1_id,
                        )?.name || "Team 1"}
                      </option>
                    )}
                    {formData.team2_id &&
                      formData.team2_id !== formData.team1_id && (
                        <option value={formData.team2_id}>
                          {teams.find(
                            (t) => t.id.toString() === formData.team2_id,
                          )?.name || "Team 2"}
                        </option>
                      )}
                  </select>
                  <p className="text-[10px] text-zinc-500 mt-1">
                    Leave empty to auto-calculate winner from score
                  </p>
                </div>
              </div>
              ...
              <div className="pt-6 flex gap-4">
                ...
                <Button
                  ...
                  className="flex-1 bg-purple-500 text-black hover:bg-purple-400"
                >
                  {isSubmitting
                    ? "Transmitting..."
                    : editingId
