...
export function TournamentStandings({
      ...
      try {
        const [ttRes, fixRes] = await Promise.all([
          ...
          supabase
            .from("fixtures")
            .select(
              "id, team1_id, team2_id, winner_team_id, status, best_of, score, stage",
            )
            .eq("tournament_id", tournamentId)
            .eq("status", "completed"),
        ]);
  ...
  const standings = useMemo(() => {
    ...
    fixtures.forEach((f) => {
      ...
      // Knockout safety: Only round-robin fixtures should affect standings calculations
      const stage = f.stage || "round-robin";
      if (stage !== "round-robin") return;
      ...
      const s1 =
        typeof f.score.team1Score === "number" ? f.score.team1Score : 0;
      const s2 =
        typeof f.score.team2Score === "number" ? f.score.team2Score : 0;

      if (f.best_of === 2) {
        if (s1 > s2 || f.winner_team_id === f.team1_id) {
          team1.wins++;
          team1.points += 3;
          team2.losses++;
        } else if (s2 > s1 || f.winner_team_id === f.team2_id) {
          team2.wins++;
          team2.points += 3;
          team1.losses++;
        } else {
        ...
        // Best of 3 (or default points rules)
        if (s1 > s2 || f.winner_team_id === f.team1_id) {
          team1.wins++;
          if (s2 === 0) {
            // 2-0 result
      ...
      <div className="overflow-x-auto">
        ...
        <thead className="text-[10px] uppercase tracking-widest text-zinc-500 bg-zinc-900/50 border-y border-zinc-800">
          <tr>
            ...
            <th className="px-2 py-3 font-semibold text-center" title="Played">
              P
            </th>
            <th className="px-2 py-3 font-semibold text-center" title="Wins">
              W
            </th>
            <th className="px-2 py-3 font-semibold text-center" title="Draws">
              D
            </th>
            <th className="px-2 py-3 font-semibold text-center" title="Losses">
              L
            </th>
            <th
              className="px-2 py-3 font-semibold text-amber-500 text-center"
              title="Points"
            >
              Pts
            </th>
            <th className="px-2 py-3 font-semibold text-center" title="Scored">
              PF
            </th>
            <th className="px-2 py-3 font-semibold text-center" title="Against">
              PA
            </th>
            <th
        ...
        <tbody>
          {standings.map((s, idx) => (
            ...
            >
              ...
              <td className="px-2 py-3 text-center text-amber-500 font-bold">
                {s.points}
              </td>
              <td className="px-2 py-3 text-center text-zinc-400">
                {s.scored}
              ...
              <td
                className={`px-2 py-3 text-center font-bold ${s.difference > 0 ? "text-emerald-500" : s.difference < 0 ? "text-red-500" : "text-zinc-500"}`}
              >
                {s.difference > 0 ? "+" : ""}
                {s.difference}
              </td>
